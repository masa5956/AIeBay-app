...
| `GET /api/research/articles` | `?q=<any keyword>`. Backs the Research tab — a "find what's trending/exportable" discovery feed, first step toward the requested export-arbitrage tool (e.g. Japan-cheap/overseas-pricey items like Nivea lip balm or Fino hair masks). No category concept server-side at all — every request, whether from a built-in category button or free-text search, is just a keyword passed straight to `searchResearchArticles` ([researchFeeds.js](server/researchFeeds.js)), which queries Currents API's `/v1/search`. **No AI call** — deliberately kept AI-free so it never burns Gemini/Groq quota. Returns latest articles (title/link/source/pubDate), capped at 30. Requires `CURRENTS_API_KEY` — throws a clear Japanese "not configured" error (surfaced to the UI) if missing, rather than crashing. |
...
| [researchFeeds.js](server/researchFeeds.js) | `searchResearchArticles(query)` — single function backing the entire Research tab (both built-in categories and free-text search funnel through this). Calls Currents API's `/v1/search` (`apiKey` as query param — confirmed live it also accepts an `Authorization` header per the OpenAPI spec, but the query-param form is what the official docs/examples use — `keywords=<query>`, `language=ja`). **Why Currents over the alternatives tried**: Google News search-RSS is scoped by Google's own feed metadata to personal/non-commercial use; GNews/Mediastack free tiers explicitly forbid commercial use (paid tiers required, ~€25–50/mo); NewsData.io's free tier does explicitly permit commercial use but only 200 credits/day with a 12h article delay. Currents' free tier also explicitly permits commercial use (limited to "processing, attribution, and link-out" per its ToS — exactly this feature's usage pattern, title+link only, no republishing/archiving) while giving 1,000 requests/day with no artificial delay on newly-indexed stories — resolves both the quota and freshness complaints NewsData.io had. Throws a clear Japanese error (surfaced to the UI, not a crash) if `CURRENTS_API_KEY` is unset. |
...
| Research tab | `CURRENTS_API_KEY` (get one at https://currentsapi.services/ — free, commercially-licensed, no card required) |
