...
| [ResearchPanel.tsx](src/components/ResearchPanel.tsx) | Research tab. A "category" is just a labeled search query (`ResearchCategoryDef = {key, label, query, isCustom}`) — 3 built-in ones (コスメ/ゲーム/ガジェット, `isCustom: false`, not deletable, deliberately single plain words with no boolean operators — see [researchFeeds.js](server/researchFeeds.js) note on why "OR" broke everything) plus any the user has saved from a free-text search (`isCustom: true`, `×` button to remove, persisted in `localStorage` under `ebay-ai-lister-research-custom-categories` so adding a new category is just typing a keyword once and tapping "＋カテゴリに追加" — no code change needed). Category selection and the free-text search box are mutually exclusive; both call the same `searchResearchArticles(query)` → `GET /api/research/articles?q=`. Article cards (title/source) are each an `<a target="_blank">` straight to the real article URL. No AI call involved (see below) — keyword search against Serper.dev (Google News results), so no Gemini/Groq quota concerns. Nothing auto-refreshes while the tab stays open — each mount/category-click/search-submit does a fresh live fetch. |
...
| `GET /api/research/articles` | `?q=<any keyword>`. Backs the Research tab — a "find what's trending/exportable" discovery feed, first step toward the requested export-arbitrage tool (e.g. Japan-cheap/overseas-pricey items like Nivea lip balm or Fino hair masks). No category concept server-side at all — every request, whether from a built-in category button or free-text search, is just a keyword passed straight to `searchResearchArticles` ([researchFeeds.js](server/researchFeeds.js)), which queries Serper.dev's `POST /news`. **No AI call** — deliberately kept AI-free so it never burns Gemini/Groq quota. Returns latest articles (title/link/source/pubDate), capped at 30. Requires `SERPER_API_KEY` — throws a clear Japanese "not configured" error (surfaced to the UI) if missing, rather than crashing. |
...
| [researchFeeds.js](server/researchFeeds.js) | `searchResearchArticles(query)` — single function backing the entire Research tab (both built-in categories and free-text search funnel through this). `POST https://google.serper.dev/news` with `X-API-KEY` header, body `{q, gl: 'jp', hl: 'ja'}` — returns actual Google News results (`news[]`: title/link/date/source). **Provider history** (each swap driven by a real problem found in live use, in order): Google News search-RSS worked great but is scoped by Google's own feed metadata to personal/non-commercial use → switched to NewsData.io (free tier explicitly permits commercial use, but only 200 credits/day + 12h article delay) → switched to Currents API (explicitly commercial-use-permitted free tier, 1,000 req/day, no artificial delay — but live testing showed very thin Japanese-language coverage, e.g. a "ゲーム" search returned only 1 article and other categories returned 0) → **settled on Serper.dev**, which resells actual Google Search/News results as its core (explicitly commercial) business model — this is the closest thing to "Google-level" JP coverage available without the personal-use ambiguity of hitting Google directly. Free tier: 2,500 queries, no card required; ¥ cheap after (~$1/1,000 queries). Throws a clear Japanese error (surfaced to the UI, not a crash) if `SERPER_API_KEY` is unset. Built-in category queries are deliberately single plain words (no `OR`/boolean syntax) — a previous version wrote category queries like `'コスメ 新作 OR 新色'` which broke everything, because none of these providers' plain keyword params treat `OR` as boolean; it's read as a literal required token, so a query almost never matches → near-empty results that looked identical across categories. |
...
| Research tab | `SERPER_API_KEY` (get one at https://serper.dev/ — free 2,500 queries, no card required, commercially-licensed) |
