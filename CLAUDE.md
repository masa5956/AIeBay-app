...
- **AI quota**: up to 4 Gemini calls per photo set (extraction, condition, price research, and the Browse-API fallback's market-trend/competitor calls only when that fallback triggers) — hits free-tier limits fast (confirmed live: this account's Gemini quota was fully exhausted (`RESOURCE_EXHAUSTED`, 429) mid-session, on both grounded and plain calls). Groq (`AI_PROVIDER=groq`/`TEXT_AI_PROVIDER=groq`) was tried as an escape hatch and is still fully wired up and working ([groqClient.js](server/groqClient.js): vision defaults to `qwen/qwen3.6-27b` since the old default `meta-llama/llama-4-scout-17b-16e-instruct` was retired by Groq and 404s now; text-only agents use a separate `GROQ_TEXT_MODEL`, default `llama-3.3-70b-versatile`, non-reasoning — routing text through the reasoning `qwen` model made `/api/estimate-price` noticeably slower for no benefit; `reasoning_format: 'hidden'` is applied only to models matching `REASONING_MODEL_PATTERN = /qwen|deepseek-r1|gpt-oss/i` since that param 400s on non-reasoning models; this account's `qwen/qwen3.6-27b` tier also has a tight 8000 TPM cap). **Currently reverted to Gemini-only** (`AI_PROVIDER=gemini`, `TEXT_AI_PROVIDER=gemini`) per user preference — flip both back to `groq` if Gemini quota becomes a recurring blocker.
