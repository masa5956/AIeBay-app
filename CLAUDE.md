...
| [ResearchPanel.tsx](src/components/ResearchPanel.tsx) | Research tab: category buttons (コスメ/ゲーム/ガジェット) → `getResearchArticles(category)`, **or** a free-text keyword box → `searchResearchArticles(query)` (any keyword not covered by the 3 fixed categories) → article cards (title/source), each an `<a target="_blank">` straight to the real article URL. Category and free-text search are mutually exclusive (picking a category clears the search box and vice versa). No AI call involved (see `/api/research/articles` below) — purely RSS-based, so no rate-limit/quota concerns. Nothing auto-refreshes while the tab stays open — each mount/category-click/search-submit does a fresh live RSS fetch, reflecting whatever the source feed has at that instant. |
...
- **API client**: [listingService.ts](src/services/listingService.ts) — `analyzeImageWithAI(files: File[])`, `estimatePrice`, `publishToEbay`, `getListings`, `getAnalytics`, `getListingDetail`, `getResearchArticles(category)`, `searchResearchArticles(query)`, `getEbayAuthUrl(env)`, `getEbayStatus`, `setActiveEbayEnv(env)`, all hitting the backend (default `http://localhost:3001/api`, override via `VITE_BACKEND_URL`). `mockAnalyzeImage`/`mockPublishItem` ([mock/mockData.ts](src/mock/mockData.ts)) back the dev mock toggle.
...
| `GET /api/research/articles` | `?category=cosmetics\|games\|gadgets` **or** `?q=<any keyword>` (mutually exclusive; `category` takes priority if both are sent). Backs the Research tab — a "find what's trending/exportable" discovery feed, first step toward the requested export-arbitrage tool (e.g. Japan-cheap/overseas-pricey items like Nivea lip balm or Fino hair masks). Fixed categories use each category's curated official RSS feed(s); free-text `q` always goes through Google News search RSS (only source that generalizes to arbitrary keywords). Returns latest articles (title/link/source/pubDate) via [researchFeeds.js](server/researchFeeds.js), **no AI call** — deliberately kept AI-free since the whole point is to avoid burning Gemini/Groq quota on a feed that refreshes constantly. |
...
| [researchFeeds.js](server/researchFeeds.js) | `getResearchArticles(category)` — fetches+parses each category's RSS feed(s) via `rss-parser`, merges, sorts by date desc, caps at 30. `RESEARCH_CATEGORIES` = `cosmetics`/`games`/`gadgets`. Games (Game*Spark) and gadgets (ITmedia PC USER, Gizmodo Japan) use each site's own official RSS. **Cosmetics has no confirmed stable official RSS** (checked live: biteki/VOCE/MAQUIA/@cosme all failed) — falls back to a Google News search-results RSS (`news.google.com/rss/search?q=...`), which Google's own feed metadata scopes to personal/non-commercial feed-reader use; fine for this local/test build, but swap to a proper source (or an official-partner API) before any commercial/production use of that category. `Promise.allSettled` per feed so one dead feed doesn't blank the whole category. `searchResearchArticles(query)` — same Google News search RSS mechanism, but for arbitrary user-typed keywords (backs the Research tab's free-text search box, since fixed per-category feeds can't cover unforeseen keywords). |
